...
/**
 * Defines UsdaArsContentPages migrate source plugin.
 *
 ...
 *
 * @MigrateSource(
 *   id = "usda_ars_content_pages",
 *   source_module = "usda_ars_migrate"
 * )
 */
class UsdaArsContentPages extends UsdaArsSource {
  ...
  public function query() {
    ...
    $query->condition('ct.alias', ['SiteStandardWebpage', 'StandardWebpage', 'Standardcolumnedwebpage'], 'IN');
    // Add parent condition.
    $query->condition('pct.alias', $this->configuration['parent_node_types'], 'IN');
    $query->condition('d.published', 1);
    // Parent has to be published as well.
    $query->condition('pd.published', 1);
    //$query->condition('c.nodeId', [189065, 189075, 189593], 'IN');
