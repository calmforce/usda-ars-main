 ...
 *
 * @MigrateSource(
 *   id = "usda_scientific_discoveries_image_cards",
 *   source_module = "usda_scientific_discoveries_migrate"
 * )
 */
class UsdaScientificDiscoveriesImageCards extends UsdaArsSource {
    ...
    if (!$row->getIdMap() || $row->needsUpdate() || $this->aboveHighwater($row) || $this->rowChanged($row)) {
      try {
        ...
        if (!empty($properties)) {
          ...
          $row->setSourceProperty('imageCardsNodeIds', $properties['imageCardsNodeIds']);
        }
      } catch (\Exception $e) {
        return FALSE;
      }
