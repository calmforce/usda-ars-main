 ...
 *
 * @MigrateSource(
 *   id = "usda_scientific_discoveries_media",
 *   source_module = "usda_scientific_discoveries_migrate"
 * )
 */
class UsdaScientificDiscoveriesMedia extends UsdaArsSource {
  ...
  public function prepareRow(Row $row) {
    if (!$row->getIdMap() || $row->needsUpdate() || $this->aboveHighwater($row) || $this->rowChanged($row)) {
      try {
        ...
        $media_type = $this->getMediaType($properties['umbracoExtension']->varcharValue);
        if (!empty($media_type)) {
