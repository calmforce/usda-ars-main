 ...
 *
 * @MigrateSource(
 *   id = "usda_tellus_slides_json_captions",
 *   source_module = "usda_tellus_migrate"
 * )
 */
class UsdaTellusSlidesJsonCaptions extends UsdaArsSource {
  ...
  public function getYield($it) {
    foreach ($it as $row) {
      ...
      $slides = $this->getSlideCaptions($row['main_content']);
      foreach ($slides as $slide) {
        $new_row = $row;
        $new_row['slideCaption'] = $slide['slide_caption'];
   ...
   */
  private function getSlideCaptions(string $json_content) {
