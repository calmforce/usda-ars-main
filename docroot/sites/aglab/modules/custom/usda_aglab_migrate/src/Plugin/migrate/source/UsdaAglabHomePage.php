...
/**
 * Defines UsdaAglabHomePage migrate source plugin.
 *
 * This annotation tells Drupal that the name of the MigrateSource plugin
 * implemented by this class is "usda_aglab_homepage". This is the name
 * that the migration configuration references with the source "plugin" key.
 *
 * @MigrateSource(
 *   id = "usda_aglab_homepage",
 *   source_module = "usda_aglab_migrate"
 * )
 */
class UsdaAglabHomePage extends UsdaArsSource {
  ...
  private function getContentComponents(string $json_content, &$properties) {
    ...
    if (!empty($areas = $content_raw["sections"][0]["rows"][0]["areas"][0]["controls"])) {
      foreach ($areas as $area) {
        if ($area['value']['dtgeContentTypeAlias'] == 'blFullScreenImage' && trim($area["value"]["value"]["title"]) == 'Welcome to AgLab') {
          // For unknown reason, instead of blHeroSlides section the blFullScreenImage is in use.
