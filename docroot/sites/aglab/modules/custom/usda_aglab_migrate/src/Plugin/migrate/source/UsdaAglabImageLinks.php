 ...
 *
 * @MigrateSource(
 *   id = "usda_aglab_image_links",
 *   source_module = "usda_aglab_migrate"
 * )
 */
class UsdaAglabImageLinks extends UsdaArsSource {
  ...
  public function query() {
    ...
    $or->condition('pd.textValue', '%imageLink":%', 'LIKE');
    $or->condition('pd.textValue', '%blImageGalleryCarousel%', 'LIKE');
    $query->condition($or);
  ...
  public function getYield($it) {
    foreach ($it as $row) {
      ...
      foreach ($imageLinks as $imageWithLink) {
        ...
        $new_row['imageCaption'] = $imageWithLink['image_caption'];
        yield($new_row);
      }
    }
  }
  ...
  private function getImageLinks(string $json_content) {
    ...
    foreach ( $sections as $index => $section) {
      ...
      elseif ($section['value']['dtgeContentTypeAlias'] == 'blImageGalleryCarousel') {
        $image_slides = $section['value']['value']['imageSlides'];
        foreach ($image_slides as $image_slide) {
          $image_uuid = $this->formatUuid(substr($image_slide['image'], 12));
          $properties[] = [
            'image_uuid' => $image_uuid,
            'image_caption' => $image_slide['text'],
            'image_link_url' => $image_slide['link'][0]['url'],
            'image_link_name' => $image_slide['link'][0]['name'],
          ];
        }
      }
    }
