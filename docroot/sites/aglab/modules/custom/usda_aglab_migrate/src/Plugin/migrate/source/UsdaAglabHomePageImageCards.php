...
/**
 * Defines UsdaAglabHomePageImageCards migrate source plugin.
 *
 * This annotation tells Drupal that the name of the MigrateSource plugin
 * implemented by this class is "usda_aglab_home_page_image_cards". This is the name
 * that the migration configuration references with the source "plugin" key.
 *
 * @MigrateSource(
 *   id = "usda_aglab_home_page_image_cards",
 *   source_module = "usda_aglab_migrate"
 * )
 */
class UsdaAglabHomePageImageCards extends UsdaArsSource {
    ...
    if (!empty($areas = $content_raw["sections"][0]["rows"][0]["areas"][0]["controls"])) {
      foreach ($areas as $index => $area) {
        ...
        if (in_array($content_type, ['blImageCards', 'blIconRow'])) {
          if ($row['nodeId'] == 1385 && $area["value"]["value"]["anchor"] != 'video') {
            continue;
          }
