...
/**
 * Defines UsdaAglabRegionsTopics migrate source plugin.
 *
 * This annotation tells Drupal that the name of the MigrateSource plugin
 * implemented by this class is "usda_aglab_regions_topics". This is the name
 * that the migration configuration references with the source "plugin" key.
 *
 * @MigrateSource(
 *   id = "usda_aglab_regions_topics",
 *   source_module = "usda_aglab_migrate"
 * )
 */
class UsdaAglabRegionsTopics extends UsdaArsSource {
      ...
      try {
        ...
        if (in_array('topicPage', $this->configuration['source_node_types'])) {
          ...
          if (!empty($topic)) {
            ...
            if ($nodeId == 1187) {
              // Fuel Your Curiosity topic.
              $row->setSourceProperty('layout', 'fuel_your_curiosity_topic');
            }
            elseif ($nodeId == 6341) {
        ...
        if (!empty($image_cards)) {
          ...
          $row->setSourceProperty('imageCardsSourceIds', $image_cards['imageCardsSourceIds']);
          $row->setSourceProperty('imageCardsSectionTitle', $image_cards['imageCardsSectionTitle']);
        }
        // SEO metatags.
